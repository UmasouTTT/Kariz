...
def load_synthetic_stream_graphs(fpath):
    ...
    with open('mdmc_sw_zipf_pig_pure100_0.5Tisolated.json', 'w') as fd:
        fd.write(json.dumps(runtime_stats))
    with open('mc_sw_zipf_pig_pure100_0Tcmr.json', 'w') as fd:
        fd.write(json.dumps(runtime_stats))



#synthetic_worload.g
load_synthetic_stream_graphs('./config/zipf_sw_mdmc_pig_pure100p.g')
