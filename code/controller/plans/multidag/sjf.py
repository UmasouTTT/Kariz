...
class SJF(Planner):
    ...
    def get_plans(self, dag_id, stage):
        ...
        sjf_dags = {}
        with self.dag_planners_mutex.reader_lock():
            if not len(self.dag_planners): return plans
            available_bw = self.remote_bw / len(self.dag_planners)  # give every Job a fair share of DAG
            for gid in self.dag_planners:
                # get remaining time
                sjf_dags[dag_id] = self.dag_planners[dag_id].get_remaining_time()

            sorted_sjf_dags = sorted(sjf_dags.items(), key=lambda x: x[1], reverse=False)
            for sorted_id in sorted_sjf_dags:
                plans.extend(self.dag_planners[sorted_id[0]].get_next_plans(available_bw))
            if dag_id in self.dag_planners:
