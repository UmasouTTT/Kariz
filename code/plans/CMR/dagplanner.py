...
class CMRScoreBoard:
    ...
    def __init__(self, stages_latencies, bandwidth):
        self.n_stages = len(stages_latencies)
        self.score_boards = {}
        
        for stgid in stages_latencies:
            self.score_boards[stgid] = sb.ScoreBoard(stages_latencies[stgid]['remote'], bandwidth)
        
        
        print(json.dumps(stages_latencies, indent=4))
        
    def update_score_board(self, stage_id, new_exect, remote_read):
        self.score_boards[stage_id].transform(new_exect)
        overflow_plans = self.score_boards[stage_id].inject_or_assert(remote_read)
        
        ''' if you can not schedule the prefetched request here just extend it '''
        for stgid in range(stage_id, self.n_stages):
            overflow_plans =  self.score_boards[stgid].inject(overflow_plans)
            if overflow_plans == 0:
                break;           
