...
class Stage:
    ...
    def get_runtime(self):
        return self.longest_job.runtime_partial_cached
    ...
    def end_time(self):
        return self.start_time + self.longest_job.runtime_remote

    def get_improvement(self): 
        if self.longest_job.runtime_cache > self.second_job.runtime_remote:
            return (self.longest_job.runtime_partial_cached - self.longest_job.runtime_cache), self.longest_job.runtime_cache
    ...
    def get_next_plan(self, priority):
        ...
        for e in self.longest_jobs:
            t_imprv_old = e.runtime_remote - e.runtime_partial_cached
            e.runtime_partial_cached = new_runtime
            if e.runtime_remote == e.runtime_cache:
                continue
            
            scale_factor =  (t_imprv_old + t_imprv)/(e.runtime_remote - e.runtime_cache)
...
class Plan:
    def __init__(self, stg_id=-1):
        ...
        self.name = self.uuid #FIXME: for now set the name to plan id, I may change it later
        self.size = 0
        self.unique_size = 0
        ...
        self.status = 0 # status could be 0, and 1 where zero means prefetched/cached and 1 means unprefetched
        self.data = {}
        self.jobs = []
        self.stage = -1
        self.feasible = 1
    ...
    def update_infeasible(self):
        self.feasible = 0
        child_plan = self.children
        while (child_plan) and (child_plan.feasible):
            child_plan.feasible = 0
            child_plan = child_plan.children

    def update_status(self):
        self.status = 1
        parent_plan = self.parent
        while (parent_plan) and (not parent_plan.status):
            parent_plan.status = 1
            parent_plan = parent_plan.parent
...
class PlansContainer:
    ...
    def add_cache_plan(self, plan, s):
        ...
        self.pc_by_uuid[plan.uuid] = plan
    ...
    def get_cache_plans(self, stage):
        ...
        if stage in self.cp_by_stage:
            for pp in self.cp_by_stage[stage]:
                ...
                self.cp_by_stage[stage][pp].type = 0
