...
def send_new_dag_rpc(serialized_stage):
    path = "/newdag"
    url = base_url + path
    reply = requests.post(url, json.dumps(serialized_stage)).text
...
def serialize_stage(g):
    return json.dumps({'id': g.gp.id, 'uuid': g.gp.uuid, 'cur_stage': g.gp.cur_stage})
...
def serialize_dag_complete(g):
    return json.dumps({'uuid': g.gp.uuid, 'id': g.gp.id})
